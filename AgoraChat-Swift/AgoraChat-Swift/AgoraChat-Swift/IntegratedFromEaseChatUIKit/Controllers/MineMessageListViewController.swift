...
extension MineMessageListViewController: ImageBrowserProtocol {
    ...
    func photo(of index: Int, with browser: ImagePreviewController) -> PreviewImage {
        if let row = self.messageContainer.messages.firstIndex(of: self.imageEntity),let cell = self.messageContainer.messageList.cellForRow(at: IndexPath(item: row, section: 0)) as? ImageMessageCell,let body = self.imageEntity.message.body as? ChatImageMessageBody,let image = UIImage(contentsOfFile: body.localPath) {
