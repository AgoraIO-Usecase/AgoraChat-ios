//
//  ACDGroupMemberMutedListViewController.swift
//  AgoraChat-Swift
//
...
class ACDGroupMemberMutedListViewController: ACDContainerSearchTableViewController<[AgoraUserModel]> {
    ...
    func updateUI() {
        if self.group.permissionType == .owner || self.group.permissionType == .admin {
            self.fetchMutes(isRefresh: true)
        }
    }
    
    private func fetchMutes(isRefresh: Bool) {
        if isRefresh {
            self.page = 1
        } else {
            self.page += 1
        }
        AgoraChatClient.shared().groupManager?.getGroupMuteListFromServer(withId: self.group.groupId, pageNumber: Int(self.page), pageSize: 20, completion: { list, error in
            self.endRefresh()
            if let error = error {
    ...
    @objc private func updateGroupMemberWithNotification(_ notification: Notification) {
        ...
        if self.group.groupId == dict["kACDGroupId"] as? String && dict["kACDGroupMemberListType"] as? ACDContainerSearchTableViewController<[AgoraUserModel]>.GroupMemberShowType == .mute {
            self.updateUI()
        }
    }
    ...
    override func didStartLoadMore() {
        self.fetchMutes(isRefresh: false)
    }
        ...
        if let cell = cell as? ACDContactCell {
            ...
            if let model = model {
                ...
                cell.tapCellHandle = { [unowned self] in
                    self.showActionSheet(userId: model.hyphenateId, showType: .mute, group: group)
                }
            }
        }
        return cell
