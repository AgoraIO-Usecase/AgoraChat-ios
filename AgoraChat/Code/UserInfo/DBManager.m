...
-(void) openDB
{
    ...
    if([AgoraChatClient sharedClient].options.appkey.length > 0) {
        NSString*appkeyPath = [path stringByAppendingPathComponent:[AgoraChatClient sharedClient].options.appkey];
        [self createPath:appkeyPath];
        if([AgoraChatClient sharedClient].currentUsername.length > 0) {
            NSString* userPath = [appkeyPath stringByAppendingPathComponent:[AgoraChatClient sharedClient].currentUsername];
...
-(void) saveUserInfos
{
    if([self.database isOpen] && self.usersInfoArray.count > 0) {
        ...
        for(AgoraChatUserInfo* userInfo in self.usersInfoArray) {
            if(bFirst) {
                bFirst = NO;
...
-(NSArray<AgoraChatUserInfo*>*) loadUserInfos
{
